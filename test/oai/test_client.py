...
def test_usage_summary():
    ...
    assert (
        client.actual_usage_summary["total_cost"] > 0
    ), "total_cost should be greater than 0"
    assert (
        client.total_usage_summary["total_cost"] > 0
    ), "total_cost should be greater than 0"
...
def test_legacy_cache():
    ...
    cold_cache_response = client.create(
    ...
    start_time = time.time()
    warm_cache_response = client.create(
        messages=[{"role": "user", "content": "random()"}]
    )
    end_time = time.time()
    ...
    assert os.path.exists(
        os.path.join(LEGACY_CACHE_DIR, str(LEGACY_DEFAULT_CACHE_SEED))
    )
    ...
    start_time = time.time()
    cold_cache_response = client.create(
        messages=[{"role": "user", "content": "random()"}], cache_seed=17
    )
    end_time = time.time()
    duration_with_cold_cache = end_time - start_time
    ...
    start_time = time.time()
    cold_cache_response = client.create(
        messages=[{"role": "user", "content": "random()"}], cache_seed=21
    )
    end_time = time.time()
    duration_with_cold_cache = end_time - start_time
    assert duration_with_warm_cache < duration_with_cold_cache
...
def test_cache():
    ...
    with Cache.disk(cache_seed=49, cache_path_root=cache_dir) as cache:
        ...
        assert not os.path.exists(
            os.path.join(cache_dir, str(LEGACY_DEFAULT_CACHE_SEED))
        )
    ...
    with Cache.disk(cache_seed=312, cache_path_root=cache_dir) as cache:
        start_time = time.time()
        cold_cache_response = client.create(
            messages=[{"role": "user", "content": "random()"}], cache=cache
        )
        end_time = time.time()
        duration_with_cold_cache = end_time - start_time
